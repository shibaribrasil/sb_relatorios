...
- **Depois da mídia (Google Ads):** `tb_gads_conta_diario` (`dbt_dw_us_az`, região US — consultada à parte, não dá para juntar na query com a `az` de us-east4). Indicadores: investimento, margem após mídia = margem de contribuição − investimento (R$ e % da receita líquida), ROAS = faturamento total ÷ investimento, custo por pedido = investimento ÷ pedidos totais. Só Google Ads; ROAS e custo por pedido são do negócio (todas as origens), não do canal. Histórico do Ads desde 15/06/2026. A margem de contribuição continua sendo a "antes de mídia"; a "após mídia" é sempre rotulada.
